...
function Parser(options) {
  ...
  var parsedTags = [
    'type', 'superClass', 'ownedMember', 'redefinedProperty',
    'ownedEnd', 'cmof:Tag', 'cmof:Package', 'ownedAttribute',
    'ownedLiteral'
  ];

  var parsedTypes = [
    'cmof:Class', 'cmof:Association', 'cmof:PrimitiveType',
    'cmof:DataType', 'cmof:Package', 'cmof:Enumeration',
    'cmof:Property'
  ];
  ...
  var collectionMap = {
    ...
    'cmof:Enumeration': 'enumerations'
  };

  function parseElement(node, context) {
    var value = {};

    forEach(node.attributes, function(val, key) {
  ...
  function parseType(node, context) {
    ...
    if (!ref) {
      console.error('no type ref found:', node);
  ...
  function parseRedefine(node, context) {
    ...
    if (!ref) {
      console.error('no ref found: ', node);
      throw new Error('no type ref');
    }
  ...
  function parseFile(file, done) {
    ...
    saxStream.on('opentag', function(node) {
      ...
      if (parsedTags.indexOf(name) !== -1) {
        ...
        if (!parser) {
          throw new Error('[modelgen] no parser for tag <' + name + '>');
        }
